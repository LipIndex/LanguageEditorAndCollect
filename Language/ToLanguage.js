                            ...
                            if (matchArr) {
                                ...
                                for (let needChangeContext of matchArr) {
                                    ...
                                    if (resultCfg) {
                                        ...
                                        // 如果代码中有"LanUtil.getLanguage(" 这种的， 直接替换 单双引号都要替换
                                        data = data.replace(`${prefix}(\"` + result[0] + "\")", `${prefix}(\"` + "STL_" + nextId + "\")");
                                        data = data.replace(`${prefix}(\"` + result[0] + "\')", `${prefix}(\"` + "STL_" + nextId + "\")");
                                    }
                                }
                            }
...
// ---------------------------------------------------------------执行逻辑分割线---------------------------------------------------------------------
function run(consoleTEXT, consoleERROR, inPrefix, rootPath, excelPath, languageExcelName, callBack) {
    try {
        prefix = inPrefix;
